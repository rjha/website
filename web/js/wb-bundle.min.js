...
webgloo.message={get:function(b,c){var a="";
if(webgloo.message.hasOwnProperty(b)){a=webgloo.message[b].supplant(c);
}return a;
}};
...
webgloo.util={addTextCounter:function(a,c,b){var d={show_counter:true};
webgloo.util.settings=$.extend({},d,b);
$(a).keydown(function(){webgloo.util.displayTextCounter(a,c);
});
if(webgloo.util.settings.show_counter){webgloo.util.displayTextCounter(a,c);
}},displayTextCounter:function(b,c){var a=$(b).attr("maxlength");
var e=$(b).val();
var d=e.length;
$(c).text(d+"/"+a);
}};
webgloo.Ajax={addSpinner:function(b){$(b).html("");
var a=webgloo.message.SPINNER;
$(b).html(a);
...
e.fail(function(f){webgloo.Ajax.show(c.messageDivId,f);
});
e.done(function(f){if(c.dataType=="json"){webgloo.Ajax.show(c.messageDivId,f.message);
}if(typeof c.onDoneHandler!=="undefined"){c.onDoneHandler(a,f);
}});
}};
webgloo.media={images:{},debug:false,counter:0,init:function(b){var c={formName:"form1",formId:"#form1",removeImageClass:"a.remove-image",previewDiv:"#image-preview",imageDiv:'<div class="container" id="image-{id}"> '+' <img src="{srcImage}" alt="{originalName}" width="{width}" height="{height}"/> '+'<div class="link"> <a class="remove-image" id="{id}" href="">Remove</a> </div> </div>'};
webgloo.media.settings=$.extend({},c,b);
frm=document.forms[webgloo.media.settings.formName];
var d=frm.media_json.value;
var a=JSON.parse(d);
for(i=0;
i<a.length;
i++){webgloo.media.addImage(a[i]);
}},attachEvents:function(){$(webgloo.media.settings.removeImageClass).live("click",function(a){a.preventDefault();
webgloo.media.removeImage($(this));
});
$(webgloo.media.settings.formId).submit(function(){webgloo.media.populateHidden();
return true;
});
},populateHidden:function(){var c=document.forms[webgloo.media.settings.formName];
var b=new Array();
$(webgloo.media.settings.previewDiv).find("a").each(function(d){var e=$(this).attr("id");
b.push(webgloo.media.images[e]);
});
var a=JSON.stringify(b);
c.media_json.value=a;
},removeImage:function(b){var c=$(b).attr("id");
var a="#image-"+c;
$("#image-"+c).remove();
webgloo.media.counter--;
},addImage:function(a){switch(a.store){case"s3":webgloo.media.counter++;
a.srcImage="http://"+a.bucket+"/"+a.thumbnail;
var b=webgloo.media.settings.imageDiv.supplant(a);
$(webgloo.media.settings.previewDiv).append(b);
webgloo.media.images[a.id]=a;
break;
case"external":webgloo.media.counter++;
a.srcImage=a.srcImage;
a.id="ext-"+webgloo.media.counter;
var b=webgloo.media.settings.imageDiv.supplant(a);
$(webgloo.media.settings.previewDiv).append(b);
webgloo.media.images[a.id]=a;
break;
case"local":webgloo.media.counter++;
a.srcImage="/"+a.bucket+"/"+a.thumbnail;
var b=webgloo.media.settings.imageDiv.supplant(a);
$(webgloo.media.settings.previewDiv).append(b);
webgloo.media.images[a.id]=a;
break;
