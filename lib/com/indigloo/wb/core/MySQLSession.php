...
namespace com\indigloo\wb\core {
    ...
    use \com\indigloo\wb\mysql\WbPdoWrapper as WbPdoWrapper;
    ...
    class MySQLSession {
        ...
        function open($path,$name) {
            ...
            $this->dbh = WbPdoWrapper::getHandle();
            return TRUE ;
        }
        ...
        function read($sessionId) {
            ...
            $sql = " select data from wb_php_session where session_id = :session_id  for update ";
        ...
        function write($sessionId,$data) {
            ...
            $sql = " select count(session_id) as total from wb_php_session where session_id = :session_id" ;
            $stmt = $this->dbh->prepare($sql);
            $stmt->bindParam(":session_id",$sessionId, \PDO::PARAM_STR);
            $stmt->execute();
            $result = $stmt->fetch(\PDO::FETCH_ASSOC);
            ...
            if($total > 0) {
                //existing session
                $sql2 = " update wb_php_session set data = :data, updated_on = now() where session_id = :session_id" ;
            } else {
                $sql2 = "insert INTO wb_php_session(session_id,data,updated_on) VALUES(:session_id, :data, now())" ;
            }
        ...
        function destroy($sessionId) {
            ...
            $sql = "DELETE FROM wb_php_session WHERE session_id = :session_id ";
            $stmt = $this->dbh->prepare($sql);
            $stmt->bindParam(":session_id",$sessionId, \PDO::PARAM_STR);
            $stmt->execute();
        ...
        function gc($age) {
            ...
            $sql = "DELETE FROM wb_php_session WHERE updated_on < (now() - INTERVAL :age SECOND) ";
            $stmt = $this->dbh->prepare($sql);
